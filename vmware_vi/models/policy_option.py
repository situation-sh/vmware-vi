...
class PolicyOption(DataObject):
    ...
    """ # noqa: E501
    id: StrictStr = Field(description="Identifier for the policy option.  This value matches one of the keys from the list of possible options in the policy metadata (*ProfilePolicyMetadata*.*ProfilePolicyMetadata.possibleOption*\\[\\].*ProfilePolicyOptionMetadata.id*.*ElementDescription.key*).  ***Since:*** vSphere API 4.0 ")
    parameter: Optional[List[KeyAnyValue]] = Field(default=None, description="Parameters for the policy option.  This list must include all parameters that are not marked as optional in the policy option metadata parameter list (*ProfilePolicyMetadata*.*ProfilePolicyMetadata.possibleOption*\\[\\].*ProfilePolicyOptionMetadata.parameter*\\[\\].*ProfileParameterMetadata.optional*).  ***Since:*** vSphere API 4.0 ")
    ...
    def from_dict(cls, obj: Dict) -> Union[Self]:
        ...
        if object_type:
            ...
            raise ValueError("PolicyOption failed to lookup discriminator value from " +
...
PolicyOption.model_rebuild(raise_errors=False)
