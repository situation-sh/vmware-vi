...
class VirtualSCSIController(VirtualController):
    ...
    """ # noqa: E501
    hot_add_remove: Optional[StrictBool] = Field(default=None, description="All SCSI controllers support hot adding and removing of devices.  This support can't be toggled in the current implementation. Therefore, this option is ignored when reconfiguring a SCSI controller and is always set to \"true\" when reading an existing configuration. ", alias="hotAddRemove")
    shared_bus: VirtualSCSISharingEnum = Field(alias="sharedBus")
    scsi_ctlr_unit_number: Optional[StrictInt] = Field(default=None, description="The unit number of the SCSI controller.  The SCSI controller sits on its own bus, so this field defines which slot the controller is using. ", alias="scsiCtlrUnitNumber")
    ...
    def from_dict(cls, obj: Dict) -> Union[Self, Self, Self, Self]:
        ...
        if object_type:
            ...
            raise ValueError("VirtualSCSIController failed to lookup discriminator value from " +
...
VirtualSCSIController.model_rebuild(raise_errors=False)
