...
class NetDhcpConfigInfoDhcpOptions(DataObject):
    """
    ...
    """ # noqa: E501
    enable: StrictBool = Field(description="Report state of dhcp client services.  ***Since:*** vSphere API 4.1 ")
    config: Optional[List[KeyValue]] = Field(default=None, description="Platform specific settings for DHCP Client.  The key part is a unique number, the value part is the platform specific configuration command. For example on Linux, BSD systems using the file dhclient.conf output would be reported at system scope: key='1', value='timeout 60;' key='2', value='reboot 10;' output reported at per interface scope: key='1', value='prepend domain-name-servers 192.0.2.1;' key='2', value='equire subnet-mask, domain-name-servers;'  ***Since:*** vSphere API 4.1 ")
