...
class HostSystemSwapConfigurationDatastoreOption(HostSystemSwapConfigurationSystemSwapOption):
    """
    Use option to indicate that a user specified datastore may be used for system swap.  ***Since:*** vSphere API 5.1 
    """ # noqa: E501
    datastore: StrictStr = Field(description="The datastore to be used with this swap option.  This value should be always set when the encapsulating option is used, otherwise a call to *HostSystem.UpdateSystemSwapConfiguration* will result in a *InvalidArgument* fault.  ***Since:*** vSphere API 5.1 ")
