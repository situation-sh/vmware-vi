...
class VirtualSCSIControllerOption(VirtualControllerOption):
    ...
    """ # noqa: E501
    num_scsi_disks: IntOption = Field(alias="numSCSIDisks")
    num_scsi_cdroms: IntOption = Field(alias="numSCSICdroms")
    num_scsi_passthrough: IntOption = Field(alias="numSCSIPassthrough")
    sharing: List[VirtualSCSISharingEnum] = Field(description="Supported shared bus modes. ")
    default_shared_index: StrictInt = Field(description="Index into sharing array specifying the default value. ", alias="defaultSharedIndex")
    hot_add_remove: BoolOption = Field(alias="hotAddRemove")
    scsi_ctlr_unit_number: StrictInt = Field(description="The unit number of the SCSI controller.  The SCSI controller sits on its own bus, so that this field defines which slot the controller will use. ", alias="scsiCtlrUnitNumber")
    ...
    def from_dict(cls, obj: Dict) -> Union[Self, Self, Self, Self]:
        ...
        if object_type:
            ...
            raise ValueError("VirtualSCSIControllerOption failed to lookup discriminator value from " +
...
VirtualSCSIControllerOption.model_rebuild(raise_errors=False)
