...
class ClusterGroupInfo(DataObject):
    """
    *ClusterGroupInfo* is the base type for all virtual machine and host groups.  All virtual machines and hosts that are part of a group must be part of the same cluster.  ***Since:*** vSphere API 4.1 
    """ # noqa: E501
    name: StrictStr = Field(description="Unique name of the group.  ***Since:*** vSphere API 4.1 ")
    user_created: Optional[StrictBool] = Field(default=None, description="Flag to indicate whether the group is created by the user or the system.  ***Since:*** vSphere API 5.0 ", alias="userCreated")
    unique_id: Optional[StrictStr] = Field(default=None, description="Unique ID for the group.  uniqueID is unique within a cluster. Groups residing in different clusters might share a uniqueID.  ***Since:*** vSphere API 6.0 ", alias="uniqueID")
    ...
    def from_dict(cls, obj: Dict) -> Union[Self, Self]:
        ...
        if object_type:
            ...
            raise ValueError("ClusterGroupInfo failed to lookup discriminator value from " +
...
ClusterGroupInfo.model_rebuild(raise_errors=False)
