...
class ProfileExpression(DataObject):
    """
    ProfileExpression
    """ # noqa: E501
    id: StrictStr = Field(description="Identifier of this expression.  The id has to be unique within a Profile. The id can be used as a key while building composite expressions.  ***Since:*** vSphere API 4.0 ")
    display_name: StrictStr = Field(description="User visible display name  ***Since:*** vSphere API 4.0 ", alias="displayName")
    negated: StrictBool = Field(description="Flag indicating if the condition of the expression should be negated.  e.g: conditions like VSwitch0 has vmnic0 connected to it can be turned into VSwitch0 doesn't have vmnic0 connected to it.  ***Since:*** vSphere API 4.0 ")
    ...
    def from_dict(cls, obj: Dict) -> Union[Self, Self]:
        ...
        if object_type:
            ...
            raise ValueError("ProfileExpression failed to lookup discriminator value from " +
...
ProfileExpression.model_rebuild(raise_errors=False)
