...
class InsufficientStandbyMemoryResource(InsufficientStandbyResource):
    ...
    """ # noqa: E501
    available: StrictInt = Field(description="The total amount of memory resource available (in bytes) on all the usable hosts in the cluster (including powered on and standby hosts).  ***Since:*** vSphere API 4.0 ")
    requested: StrictInt = Field(description="The additional amount of memory resource (other than that on the hosts included in \"available\") needed (in bytes).  ***Since:*** vSphere API 4.0 ")
