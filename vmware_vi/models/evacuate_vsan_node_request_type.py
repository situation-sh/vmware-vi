...
class EvacuateVsanNodeRequestType(BaseModel):
    """
    The parameters of *HostVsanSystem.EvacuateVsanNode_Task*. 
    """ # noqa: E501
    maintenance_spec: HostMaintenanceSpec = Field(alias="maintenanceSpec")
    timeout: StrictInt = Field(description="Time to wait for the task to complete in seconds. If the value is less than or equal to zero, there is no timeout. The operation fails with a Timedout exception if it timed out. ")
    __properties: ClassVar[List[str]] = ["maintenanceSpec", "timeout"]
