...
class VirtualMachineConnection(DataObject):
    """
    The *VirtualMachineConnection* object describes a connection to the virtual machine.  ***Since:*** vSphere API 7.0.1.0 
    """ # noqa: E501
    label: StrictStr = Field(description="The unique identifier associated with the connection.  The label is a UTF-8 string which specifies a unique identifier for a connection.  ***Since:*** vSphere API 7.0.1.0 ")
    client: StrictStr = Field(description="The client identifer.  This identifier is a UTF-8 string which is semantically meaningful for the connection. Examples of the client identifier are an IP address (V4 or V6) with or without a port specification, a machine name that requires a DNS lookup, or any other network oriented identification scheme.  ***Since:*** vSphere API 7.0.1.0 ")
    user_name: StrictStr = Field(description="The name of the user authorizing the connection.  This is used for auditing.  ***Since:*** vSphere API 7.0.1.0 ", alias="userName")
    ...
    def from_dict(cls, obj: Dict) -> Union[Self]:
        ...
        if object_type:
            ...
            raise ValueError("VirtualMachineConnection failed to lookup discriminator value from " +
...
VirtualMachineConnection.model_rebuild(raise_errors=False)
