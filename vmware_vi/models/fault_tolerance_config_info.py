...
class FaultToleranceConfigInfo(DataObject):
    """
    ...
    """ # noqa: E501
    role: StrictInt = Field(description="The index of the current VM in instanceUuids array starting from 1, so 1 means that it is the primary VM.  ***Since:*** vSphere API 4.0 ")
    instance_uuids: List[StrictStr] = Field(description="The instanceUuid of all the VMs in this fault tolerance group.  The first element is the instanceUuid of the primary VM.  ***Since:*** vSphere API 4.0 ", alias="instanceUuids")
    config_paths: List[StrictStr] = Field(description="The configuration file path for all the VMs in this fault tolerance group.  ***Since:*** vSphere API 4.0 ", alias="configPaths")
    orphaned: Optional[StrictBool] = Field(default=None, description="Indicates whether a secondary VM is orphaned (no longer associated with the primary VM).  ***Since:*** vSphere API 6.0 ")
    ...
    def from_dict(cls, obj: Dict) -> Union[Self, Self]:
        ...
        if object_type:
            ...
            raise ValueError("FaultToleranceConfigInfo failed to lookup discriminator value from " +
...
FaultToleranceConfigInfo.model_rebuild(raise_errors=False)
