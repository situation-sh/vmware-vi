...
class OvfConsumerOstNode(DataObject):
    """
    ...
    """ # noqa: E501
    id: StrictStr = Field(description="The OVF id of the Content (VirtualSystem or VirtualSystemCollection) element.  Empty on the envelope node.  ***Since:*** vSphere API 5.0 ")
    type: StrictStr = Field(description="The type of the node.  Possible values are defined in the OstNodeType enum.  Since the OstNode tree structure mirrors the structure of the OVF descriptor, only one Envelope node is defined, and it is always the root of the tree.  ***Since:*** vSphere API 5.0 ")
    section: Optional[List[OvfConsumerOvfSection]] = Field(default=None, description="The list of sections on this node.  ***Since:*** vSphere API 5.0 ")
    child: Optional[List[OvfConsumerOstNode]] = Field(default=None, description="The list of child nodes.  As dictated by OVF, this list is subject to the following rules: - The Envelope node must have exactly one child. - VirtualSystemCollection nodes may have zero or more children. - VirtualSystem nodes must have no children.    ***Since:*** vSphere API 5.0 ")
...
OvfConsumerOstNode.model_rebuild(raise_errors=False)
