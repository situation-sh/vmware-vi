...
class AlarmSpec(DataObject):
    """
    Parameters for alarm creation. 
    """ # noqa: E501
    name: StrictStr = Field(description="Name of the alarm. ")
    system_name: Optional[StrictStr] = Field(default=None, description="System name of the alarm.  This is set only for predefined Alarms - i.e. Alarms created by the server or extensions automatically. After creation this value cannot be modified. User-created Alarms do not have a systemName at all.  The purpose of this field is to identify system-created Alarms reliably, even if they are edited by users.  When creating Alarms with systemName, the systemName and the name of the alarm should be equal.  When reconfiguring an Alarm with systemName, the same systemName should be passed in the new AlarmSpec. Renaming Alarms with systemName is not allowed, i.e. when reconfiguring, the name passed in the new AlarmSpec should be equal to either the systemName or its localized version (the current name in the Alarm's info).  ***Since:*** vSphere API 5.0 ", alias="systemName")
    description: StrictStr = Field(description="Description of the alarm. ")
    enabled: StrictBool = Field(description="Flag to indicate whether or not the alarm is enabled or disabled. ")
    expression: AlarmExpression
    action: Optional[AlarmAction] = None
    action_frequency: Optional[StrictInt] = Field(default=None, description="Frequency in seconds, which specifies how often appropriate actions should repeat when an alarm does not change state.  ***Since:*** vSphere API 4.0 ", alias="actionFrequency")
    ...
    def from_dict(cls, obj: Dict) -> Union[Self]:
        ...
        if object_type:
            ...
            raise ValueError("AlarmSpec failed to lookup discriminator value from " +
...
AlarmSpec.model_rebuild(raise_errors=False)
