...
class RemoveDiskMappingRequestType(BaseModel):
    """
    The parameters of *HostVsanSystem.RemoveDiskMapping_Task*. 
    """ # noqa: E501
    mapping: List[VsanHostDiskMapping] = Field(description="list of disk mappings to be removed from VSAN usage.  ***Since:*** vSphere API 5.5 ")
    maintenance_spec: Optional[HostMaintenanceSpec] = Field(default=None, alias="maintenanceSpec")
    timeout: Optional[StrictInt] = Field(default=None, description="Time to wait for the task to complete in seconds. If the value is less than or equal to zero, there is no timeout. The operation fails with a Timedout exception if it timed out. ")
    __properties: ClassVar[List[str]] = ["mapping", "maintenanceSpec", "timeout"]
    ...
    def from_dict(cls, obj: Dict) -> Self:
        ...
        _obj = cls.model_validate({
            "mapping": [VsanHostDiskMapping.from_dict(_item) for _item in obj.get("mapping")] if obj.get("mapping") is not None else None,
