...
class DvsOutOfSyncHostArgument(DataObject):
    """
    The host on which the DVS configuration is different from that of Virtual Center server.  ***Since:*** vSphere API 4.0 
    """ # noqa: E501
    out_of_sync_host: HostEventArgument = Field(alias="outOfSyncHost")
    config_paramters: List[StrictStr] = Field(description="The DVS configuration parameters that are different between Virtual Center server and the host.  ***Since:*** vSphere API 4.0 ", alias="configParamters")
