...
class DistributedVirtualSwitchManagerHostDvsFilterSpec(DataObject):
    """
    Base class for filters to check host compatibility.  ***Since:*** vSphere API 4.1 
    """ # noqa: E501
    inclusive: StrictBool = Field(description="If this flag is true, then the filter returns the hosts in the *DistributedVirtualSwitchManagerHostContainer* that satisfy the criteria specified by this filter, otherwise it returns hosts that don't meet the criteria.  ***Since:*** vSphere API 4.1 ")
    ...
    def from_dict(cls, obj: Dict) -> Union[Self, Self, Self]:
        ...
        if object_type:
            ...
            raise ValueError("DistributedVirtualSwitchManagerHostDvsFilterSpec failed to lookup discriminator value from " +
...
DistributedVirtualSwitchManagerHostDvsFilterSpec.model_rebuild(raise_errors=False)
