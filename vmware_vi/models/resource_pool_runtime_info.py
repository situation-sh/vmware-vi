...
class ResourcePoolRuntimeInfo(DataObject):
    """
    ...
    shares_scalable: Optional[StrictStr] = Field(default=None, description="The scaling behavior of the shares of a given resource pool.  See *ResourceConfigSpecScaleSharesBehavior_enum* for possible values. The system will automatically compute this property based on the *ResourceConfigSpec.scaleDescendantsShares* setting on every ancestor resource pool. This property does not apply to virtual machines.  ***Since:*** vSphere API 7.0 ", alias="sharesScalable")
