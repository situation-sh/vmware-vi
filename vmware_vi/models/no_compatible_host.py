...
class NoCompatibleHost(VimFault):
    ...
    """ # noqa: E501
    host: Optional[List[ManagedObjectReference]] = Field(default=None, description="The list of hosts that are not compatible, each element has a corresponding fault in the error array.  ***Since:*** vSphere API 4.0  Refers instances of *HostSystem*. ")
    error: Optional[List[MethodFault]] = Field(default=None, description="An error in this array indicates why the corresponding host in the host array is incompatible.  ***Since:*** vSphere API 4.0 ")
    ...
    def from_dict(cls, obj: Dict) -> Union[Self]:
        ...
        if object_type:
            ...
            raise ValueError("NoCompatibleHost failed to lookup discriminator value from " +
...
NoCompatibleHost.model_rebuild(raise_errors=False)
