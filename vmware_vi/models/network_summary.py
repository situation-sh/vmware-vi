...
class NetworkSummary(DataObject):
    """
    General information about a network. 
    """ # noqa: E501
    network: Optional[ManagedObjectReference] = None
    name: StrictStr = Field(description="Name of the network. ")
    accessible: StrictBool = Field(description="At least one host is configured to provide this network. ")
    ip_pool_name: StrictStr = Field(description="Name of the associated IP pool.  Empty if the network is not associated with an IP pool.  ***Since:*** vSphere API 4.0 ", alias="ipPoolName")
    ip_pool_id: Optional[StrictInt] = Field(default=None, description="Identifier of the associated IP pool.  Zero if the network is not associated with an IP pool.  ***Since:*** vSphere API 5.1 ", alias="ipPoolId")
    ...
    def from_dict(cls, obj: Dict) -> Union[Self]:
        ...
        if object_type:
            ...
            raise ValueError("NetworkSummary failed to lookup discriminator value from " +
...
NetworkSummary.model_rebuild(raise_errors=False)
