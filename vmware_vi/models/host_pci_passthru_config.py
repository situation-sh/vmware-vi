...
class HostPciPassthruConfig(DataObject):
    """
    ...
    passthru_enabled: StrictBool = Field(description="Whether passThru has been configured for this device  ***Since:*** vSphere API 4.0 ", alias="passthruEnabled")
    apply_now: Optional[StrictBool] = Field(default=None, description="Whether the passThru config should take effect without rebooting ESX.  When unset, the behavior will be determined automatically based on *HostCapability.deviceRebindWithoutRebootSupported*. If the configuration can be applied immediately, it will be, otherwise the changes will take effect after reboot.  ***Since:*** vSphere API 7.0 ", alias="applyNow")
    hardware_label: Optional[StrictStr] = Field(default=None, description="The hardware label of the this PCI device.  ***Since:*** vSphere API 7.0.2.0 ", alias="hardwareLabel")
    ...
    def from_dict(cls, obj: Dict) -> Union[Self]:
        ...
        if object_type:
            ...
            raise ValueError("HostPciPassthruConfig failed to lookup discriminator value from " +
...
HostPciPassthruConfig.model_rebuild(raise_errors=False)
