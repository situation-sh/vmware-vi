...
class VirtualDeviceURIBackingInfo(VirtualDeviceBackingInfo):
    ...
    """ # noqa: E501
    service_uri: StrictStr = Field(description="Identifies the local host or a system on the network, depending on the value of <code>*VirtualDeviceURIBackingInfo.direction*</code>. - If you use the virtual machine as a server, the URI identifies   the host on which the virtual machine runs. In this case,   the host name part of the URI should be empty, or it should   specify the address of the local host. - If you use the virtual machine as a client, the URI identifies   the remote system on the network.    ***Since:*** vSphere API 4.1 ", alias="serviceURI")
    direction: StrictStr = Field(description="The direction of the connection.  For possible values see *VirtualDeviceURIBackingOptionDirection_enum*  ***Since:*** vSphere API 4.1 ")
    proxy_uri: Optional[StrictStr] = Field(default=None, description="Identifies a proxy service that provides network access to the <code>*VirtualDeviceURIBackingInfo.serviceURI*</code>.  If you specify a proxy URI, the virtual machine initiates a connection with the proxy service and forwards the *VirtualDeviceURIBackingInfo.serviceURI* and *VirtualDeviceURIBackingInfo.direction* to the proxy.  ***Since:*** vSphere API 4.1 ", alias="proxyURI")
    ...
    def from_dict(cls, obj: Dict) -> Union[Self]:
        ...
        if object_type:
            ...
            raise ValueError("VirtualDeviceURIBackingInfo failed to lookup discriminator value from " +
...
VirtualDeviceURIBackingInfo.model_rebuild(raise_errors=False)
