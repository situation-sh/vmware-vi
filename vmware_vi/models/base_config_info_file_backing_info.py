...
class BaseConfigInfoFileBackingInfo(BaseConfigInfoBackingInfo):
    """
    ...
    """ # noqa: E501
    file_path: StrictStr = Field(description="Full file path for the host file used in this backing.  ***Since:*** vSphere API 6.5 ", alias="filePath")
    backing_object_id: Optional[StrictStr] = Field(default=None, description="Id refers to the backed storage object where the virtual storage object is backed on.  ***Since:*** vSphere API 6.5 ", alias="backingObjectId")
    parent: Optional[BaseConfigInfoFileBackingInfo] = None
    delta_size_in_mb: Optional[StrictInt] = Field(default=None, description="Size allocated by the FS for this file/chain/link/extent only.  This property is used only for a delta disk whose *BaseConfigInfoFileBackingInfo.parent* is set.  ***Since:*** vSphere API 6.5 ", alias="deltaSizeInMB")
    ...
    def from_dict(cls, obj: Dict) -> Union[Self, Self]:
        ...
        if object_type:
            ...
            raise ValueError("BaseConfigInfoFileBackingInfo failed to lookup discriminator value from " +
...
BaseConfigInfoFileBackingInfo.model_rebuild(raise_errors=False)
