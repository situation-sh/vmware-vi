...
class ProfilePolicyMetadata(DataObject):
    """
    ...
    possible_option: List[ProfilePolicyOptionMetadata] = Field(description="Possible policy options that can be set for a policy of the given kind.  *HostProfile*s and subprofiles will contain selected policy options from this list. See *PolicyOption*.  ***Since:*** vSphere API 4.0 ", alias="possibleOption")
