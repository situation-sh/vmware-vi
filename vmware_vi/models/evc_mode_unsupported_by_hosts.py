...
class EVCModeUnsupportedByHosts(EVCConfigFault):
    ...
    host: Optional[List[ManagedObjectReference]] = Field(default=None, description="The set of hosts which are blocking EVC because their CPU hardware does not support the requested EVC mode.  ***Since:*** vSphere API 4.0  Refers instances of *HostSystem*. ")
