...
class ComputeResourceConfigInfo(DataObject):
    """
    Configuration of the compute resource; applies to both standalone hosts and clusters.  ***Since:*** VI API 2.5 
    """ # noqa: E501
    vm_swap_placement: StrictStr = Field(description="Swapfile placement policy for virtual machines within this compute resource.  Any policy except for \"inherit\" is a valid value for this property; the default is \"vmDirectory\". This setting will be honored for each virtual machine within the compute resource for which the following is true: - The virtual machine is executing on a host that has the   *perVmSwapFiles* capability. - The virtual machine configuration's   *swapPlacement* property is set   to \"inherit\".    See also *VirtualMachineConfigInfoSwapPlacementType_enum*.  ***Since:*** VI API 2.5 ", alias="vmSwapPlacement")
    ...
    def from_dict(cls, obj: Dict) -> Union[Self]:
        ...
        if object_type:
            ...
            raise ValueError("ComputeResourceConfigInfo failed to lookup discriminator value from " +
...
ComputeResourceConfigInfo.model_rebuild(raise_errors=False)
