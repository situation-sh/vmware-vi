...
class VmMessageErrorEvent(VmEvent):
    ...
    """ # noqa: E501
    message: StrictStr = Field(description="A raw message returned by the virtualization platform.  ***Since:*** vSphere API 4.0 ")
    message_info: Optional[List[VirtualMachineMessage]] = Field(default=None, description="A set of localizable message data that comprise this event.  Only available on servers that support localization.  ***Since:*** vSphere API 4.0 ", alias="messageInfo")
