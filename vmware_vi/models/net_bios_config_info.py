...
class NetBIOSConfigInfo(DataObject):
    """
    This data object type describes the NetBIOS configuration of an operating system.  ***Since:*** vSphere API 4.1 
    """ # noqa: E501
    mode: StrictStr = Field(description="NetBIOS configuration mode.  The supported values are described by *NetBIOSConfigInfoMode_enum*.  ***Since:*** vSphere API 4.1 ")
    ...
    def from_dict(cls, obj: Dict) -> Union[Self]:
        ...
        if object_type:
            ...
            raise ValueError("NetBIOSConfigInfo failed to lookup discriminator value from " +
...
NetBIOSConfigInfo.model_rebuild(raise_errors=False)
