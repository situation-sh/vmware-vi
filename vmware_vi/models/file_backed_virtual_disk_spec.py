...
class FileBackedVirtualDiskSpec(VirtualDiskSpec):
    ...
    """ # noqa: E501
    capacity_kb: StrictInt = Field(description="Specify the capacity of the virtual disk in Kb.  ***Since:*** VI API 2.5 ", alias="capacityKb")
    profile: Optional[List[VirtualMachineProfileSpec]] = Field(default=None, description="Virtual Disk Profile requirement.  Profiles are solution specifics. Profile Based Storage Management is a vSphere server extension. The API users who want to provision VMs using Storage Profiles, need to interact with it. This is an optional parameter and if user doesn't specify profile, the default behavior will apply.  ***Since:*** vSphere API 5.5 ")
    ...
    def from_dict(cls, obj: Dict) -> Union[Self]:
        ...
        if object_type:
            ...
            raise ValueError("FileBackedVirtualDiskSpec failed to lookup discriminator value from " +
...
FileBackedVirtualDiskSpec.model_rebuild(raise_errors=False)
