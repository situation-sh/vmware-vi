...
class InsufficientStandbyCpuResource(InsufficientStandbyResource):
    ...
    """ # noqa: E501
    available: StrictInt = Field(description="The total amount of CPU resource available (in MHz) on all the usable hosts in the cluster (including powered on and standby hosts).  ***Since:*** vSphere API 4.0 ")
    requested: StrictInt = Field(description="The additional amount of CPU resource (other than that on the hosts included in \"available\") needed (in MHz).  ***Since:*** vSphere API 4.0 ")
