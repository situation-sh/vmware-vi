...
class VsanUpgradeSystemPreflightCheckResult(DataObject):
    """
    Captures the result of a VSAN upgrade pre-flight check.  ***Since:*** vSphere API 6.0 
    """ # noqa: E501
    issues: Optional[List[VsanUpgradeSystemPreflightCheckIssue]] = Field(default=None, description="Detected issues.  In some cases, not all possible issues are captured, i.e. only the first (few) issues may be captured, and only once those are resolved would additional issues be reported. Absence of issues means the pre-flight check passed.  ***Since:*** vSphere API 6.0 ")
    disk_mapping_to_restore: Optional[VsanHostDiskMapping] = Field(default=None, alias="diskMappingToRestore")
